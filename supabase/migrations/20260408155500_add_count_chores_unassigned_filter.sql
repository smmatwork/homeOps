-- Add an 'unassigned' filter to count_chores so callers can count chores where helper_id is null
...
begin
  ...
  unassigned := (coalesce(f->>'unassigned', '') = 'true');
  ...
  if (not unassigned) and v_helper_id is null and helper_q is not null then
  ...
  where c.household_id = p_household_id
    ...
    and (not unassigned or c.helper_id is null)
    and (unassigned or v_helper_id is null or c.helper_id = v_helper_id)
